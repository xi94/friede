...
auto Client::for_each_riot_process(const std::function<bool(const PROCESSENTRY32W &)> &callback) const -> Result<void> {
    Scoped_Handle snapshot(CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0), &::CloseHandle);
    ...
    if (!Process32FirstW(snapshot.get(), &process_entry)) {
        // This can happen if no processes are running, which is not an error.
        // Check GetLastError() if a distinction is needed.
        return {};
    }
        ...
        for (const auto target_name : RIOT_PROCESS_NAMES) {
            if (target_name == process_entry.szExeFile) {
                if (!callback(process_entry)) {
                    return {}; // Callback requested to stop iteration.
                }
                break; // Found a match, move to the next process entry.
            }
        }
